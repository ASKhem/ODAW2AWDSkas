...
public class MovimientoController {
    ...
    public String createMovimientoSubmit(@PathVariable String iban, @Valid Movimiento movimiento, BindingResult bindingResult, Model model) {
        if (bindingResult.hasErrors()) {
            ...
            if(movimientoService.createMovimiento(cuentaService.getCuenta(iban),movimiento, userService.getCurrentUserRole()) == null) {
                model.addAttribute("error", "No tienes permisos para retirar dinero.");
                return "newMovement";
            }
